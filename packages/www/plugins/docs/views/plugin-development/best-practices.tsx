...
const configurationValidationExample = [
  ...
  if (!config.output || typeof config.output !== 'string') {
    throw new Error(
      'Plugin requires "output" configuration as string'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'Plugin requires "format" to be "typescript" or ' +
      '"javascript"'
    );
  }
}

export default async function validatedPlugin(
  props: PluginProps<{}>
) {
...
export function Head(props: ServerPageProps<ServerConfigProps>) {
  ...
  const description = _(
    'Essential best practices for plugin development covering type '
    + 'safety, configuration validation, file operations, and CLI '
    + 'integration'
  );
  return (
    <>
