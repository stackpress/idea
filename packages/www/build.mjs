...
const fragmentsDir = path.join(currentDir, 'fragments');
const templatesDir = path.join(currentDir, 'templates');
const stylesDir = path.join(currentDir, 'styles');
const templateCache = new Map();
const siteOrigin = 'https://www.stackpress.io';
const siteBasePath = '/idea/';
const siteName = 'Idea Documentation';
const defaultDescription = 'Idea schema language and toolchain documentation.';
...
async function build() {
  ...
  await copyDirectory(assetsDir, path.join(docsDir, 'assets'));
...
async function loadTemplate(name) {
  if (!templateCache.has(name)) {
    ...
    templateCache.set(name, await fs.readFile(templatePath, 'utf8'));
  }
...
async function collectDocs(dir) {
  ...
  for (const entry of entries) {
    ...
    const siteRel = specToSiteRel(sourceRel);
...
function orderDocs(docs) {
  ...
  return docs.sort((a, b) => {
    const weightA = weights.has(a.sourceRel) ? weights.get(a.sourceRel) : 1000;
    const weightB = weights.has(b.sourceRel) ? weights.get(b.sourceRel) : 1000;
    if (weightA !== weightB) {
...
async function renderHomePage(docs) {
  ...
  const cards = (await Promise.all(grouped.map(async(section) => {
    ...
    return renderFileTemplate('nav-card.html', {
  ...
  ].map((item) => renderFileTemplate('workflow-card.html', {
    title: item.title,
...
function wrapPage({
  ...
  bodyClass,
  main,
  currentSiteRel = 'index.html'
}) {
  const canonicalUrl = buildAbsoluteUrl(pagePath);
  const ogImageUrl = buildAbsoluteUrl('assets/brand/idea.png');
  ...
  return renderFileTemplate('layout.html', {
    ...
    ogDescription: escapeHtml(pageDescription),
    ogUrl: escapeAttribute(canonicalUrl),
    ogImageUrl: escapeAttribute(ogImageUrl),
    twitterTitle: escapeHtml(pageTitle),
    twitterDescription: escapeHtml(pageDescription),
    twitterImageUrl: escapeAttribute(ogImageUrl),
    bodyClass,
...
function buildAbsoluteUrl(pagePath = '') {
  const base = new URL(siteBasePath, `${siteOrigin}/`);
...
async function renderSidebar(docs, currentDoc) {
  ...
  const groups = await Promise.all(grouped.map(async(section) => {
    const items = (await Promise.all(section.items.map((doc) => renderFileTemplate('sidebar-link.html', {
    ...
    return renderFileTemplate('sidebar-group.html', {
      title: escapeHtml(section.title),
      items
    });
  }));

  return renderFileTemplate('sidebar.html', {
    groups: groups.join('')
  });
}
...
async function renderToc(headings) {
  if (!headings.length) {
    return renderFileTemplate('toc-empty.html', {});
  }

  const items = (await Promise.all(headings
    .filter((heading) => heading.level <= 3)
    .map((heading) => renderFileTemplate('toc-item.html', {
      id: heading.id,
      text: escapeHtml(toPlainText(heading.text))
    })))).join('');

  return renderFileTemplate('toc.html', { items });
}

async function renderPager(currentDoc, previous, next) {
  const previousHtml = previous
    ? await renderFileTemplate('pager-link.html', {
  ...
  return renderFileTemplate('pager.html', {
